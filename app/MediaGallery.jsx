...
const MediaBankGallery = () => {
  ...
  const itemWidth =
    (width * (1 - totalHorizontalPadding) - (numColumns - 1) * itemMargin) /
    numColumns;
  ...
  const uploadMedia = async () => {
    ...
    try {
      const fileName = `${Date.now()}-${
        pickedMedia.fileName || 'media'
      }.${pickedMedia.uri.split('.').pop()}`;
      const file = {
        uri: pickedMedia.uri,
        name: fileName,
      ...
      if (insertError) {
        console.error(
          'Error inserting into media_bank table:',
          insertError.message
        );
        setUploading(false);
        return;
                ...
                {pickedMedia.mediaType === 'audio' && (
                  <View style={styles.audioPreviewContainer}>
                    <Text style={styles.audioPreviewText}>
                      {pickedMedia.fileName || 'Audio File'}
                    </Text>
                    <TouchableOpacity
                      onPress={() => playAudio(pickedMedia.uri)}
                    >
